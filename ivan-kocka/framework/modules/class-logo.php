	...
	public static function display( $avoidAlternativeLogo = false ) {
		...
		if( ivan_get_option( 'logo-retina') != null ) {
			...
			if( true == isset( $logo_hd_temp['url'] ) AND '' != $logo_hd_temp['url'] )
				$logo_hd = '<img class="hd-res logo-normal" src="'. $logo_hd_temp['url'] . '" width="'. $logo['width'] .'" height="'. $logo['height'] .'" style="width:'. $logo['width'].'px;max-height:'. $logo['height'].'px;" alt="'. get_bloginfo( 'name' ) .'" />';
			...
			if( $logo_alt != null ) {
				...
				if( true == isset( $logo_alt['url'] ) && $logo_alt['url'] != '' ) {
					...
					if( ivan_get_option( 'logo-retina-' . $_scheme ) != null ) {
						...
						if( true == isset( $logo_hd_temp['url'] ) AND '' != $logo_hd_temp['url'] )
							$logo_hd_alt = '<img class="hd-res logo-alt" src="'. $logo_hd_temp['url'] . '" width="'. $logo_alt['width'] .'" height="'. $logo_alt['height'] .'" style="width:'. $logo_alt['width'].'px;max-height:'. $logo_alt['height'].'px;" alt="'. get_bloginfo( 'name' ) .'" />';
